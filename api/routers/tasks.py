...
def add_task(body: TaskCreate, user: dict = Depends(get_current_user)):
    ...
    added = user_store.add_tasks([{"task": body.task, "due": body.due}])
    user_store.invalidate_today_digests(store.registry.tz(user["user_id"]))
...
def complete_task(task_id: str, user: dict = Depends(get_current_user)):
    user_store = get_store(user["user_id"])
    done = user_store.complete_task_by_id(task_id)
    if done is None:
...
def reopen_task(task_id: str, user: dict = Depends(get_current_user)):
    user_store = get_store(user["user_id"])
    reopened = user_store.reopen_task(task_id)
    if reopened is None:
